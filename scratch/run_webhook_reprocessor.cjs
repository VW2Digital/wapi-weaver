        ...
        for (const change of entry.changes ?? []) {
          if (change.field === "messages") {
            ...
            for (const m of messages) {
              ...
              console.log(`Message Details: id=${m.id}, from=${m.from}, phoneDigits=${phoneDigits}, type=${m.type}`);
              
              // Let's check if contact exists
              const [contacts] = await conn.query("SELECT id, name, custom_fields FROM contacts WHERE user_id = ? AND phone_e164 = ?", [userId, phoneDigits]);
              const existingContact = contacts[0];
              console.log("Existing contact in db:", existingContact ? existingContact.name : "None");
              
              const existingCustomFields = existingContact?.custom_fields 
                ? (typeof existingContact.custom_fields === 'string' ? JSON.parse(existingContact.custom_fields) : existingContact.custom_fields)
                : {};
