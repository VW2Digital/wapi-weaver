...
META_CREDENTIALS_ENCRYPTION_KEY="${META_CREDENTIALS_ENC_KEY_VAL}"

# Credenciais Meta (App ID, App Secret, Config ID, Verify Token) são cadastradas
# por tenant em Configurações > Integrações Meta (meta_app_connections).
# Não armazene segredos Meta globais neste arquivo.
...
# Credenciais Meta
#
# App ID, App Secret, Verify Token e Config ID são cadastrados por tenant em
# Configurações > Integrações Meta (meta_app_connections). O installer NÃO
# gera nem sincroniza segredos Meta globais.
#
# A única chave de criptografia de infraestrutura exigida é
# META_CREDENTIALS_ENCRYPTION_KEY, já gravada no .env acima.
# ==============================================================================
print_ok "Configuração de criptografia de credenciais Meta presente no .env."
