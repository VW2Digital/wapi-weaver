...
export const togglePinContact = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    try {
      await db.query(
        "UPDATE contacts SET is_pinned = ? WHERE id = ? AND user_id = ?",
        [data.isPinned ? 1 : 0, data.contactId, context.userId]
      );
      return { ok: true };
    } catch (e: any) {
...
export const toggleArchiveContact = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    try {
      await db.query(
        "UPDATE contacts SET is_archived = ? WHERE id = ? AND user_id = ?",
        [data.isArchived ? 1 : 0, data.contactId, context.userId]
      );
      return { ok: true };
    } catch (e: any) {
...
export const updateChatStatus = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    try {
      await db.query(
        "UPDATE contacts SET chat_status = ? WHERE id = ? AND user_id = ?",
        [data.status, data.contactId, context.userId]
      );
      return { ok: true };
    } catch (e: any) {
...
export const toggleUnreadContact = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    try {
      await db.query(
        "UPDATE contacts SET is_unread = ? WHERE id = ? AND user_id = ?",
        [data.isUnread ? 1 : 0, data.contactId, context.userId]
      );
      return { ok: true };
    } catch (e: any) {
...
export const setContactKanbanStage = createServerFn({ method: "POST" })
  .middleware([requireAuth])
  .validator((d) => z.object({ contactId: z.string().min(1), stageId: z.string().min(1).nullable() }).parse(d))
  .handler(async ({ data, context }) => {
    try {
      ...
      await db.query(
        ...
        [data.stageId, data.contactId, context.userId]
      );
      return { ok: true };
    } catch (e: any) {
