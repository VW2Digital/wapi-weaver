...
function getModels(provider = null) {
  const cmd = provider ? `opencode models "${provider}"` : `opencode models`;
...
function outputSubProviderModels(provider, subProvider) {
  ...
  if (filtered.length === 0) {
    console.error(
      `Error: No models found for sub-provider "${subProvider}" in provider "${provider}".`,
    );
...
if (providersOnlyIndex !== -1 && providerIndex !== -1) {
  ...
  console.error(
    'Use --providers-only to discover providers, then --provider "name" to see models.',
  );
  process.exit(1);
}
