...
async function run() {
  ...
  const [users] = await conn.query('SELECT u.id, u.email, r.role FROM users u LEFT JOIN user_roles r ON u.id = r.user_id');
  ...
  if (targetUser) {
    await conn.query(`INSERT INTO user_roles (id, user_id, role) VALUES (UUID(), ?, 'admin') ON DUPLICATE KEY UPDATE role='admin'`, [targetUser.id]);
