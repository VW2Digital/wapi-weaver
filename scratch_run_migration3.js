...
async function run() {
  const conn = await mysql.createConnection({
    host: 'localhost', user: 'wapi_user', password: 'S0xbxPfKazBVT8JFy1UEOjIsrjox', database: 'wapi_weaver', multipleStatements: true
  });
  
  try {
    await conn.query("DROP TABLE IF EXISTS instagram_webhook_events;");
    await conn.query("DROP TABLE IF EXISTS instagram_accounts;");
